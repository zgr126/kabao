...
export default {
  ...
  methods: {
    ...
    initball(){
      ...
      let p = new THREE.SphereGeometry(.5,10,10)
      ...
      for (let i = 0;i<100;i++){
        for (let j = 0;j<100;j++){
          let _p = mesh.clone()
          _p.position.set((i-50)*10, 0, (j-50)*10)
          g.add(_p)
        }
      }
      scene.add(g)
    ...
    render(){
      ...
      TWEEN.update();
      let box = scene.getChildByName('balls')
      if (box){
        let time = new Date().getTime()
        for (let i of box.children){
          let _s = i.position.x + i.position.z
          i.position.y = Math.sin(_s/100 + time/1000)*15
        }
      }
